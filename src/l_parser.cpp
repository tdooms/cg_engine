...
void ReplacementRule::addRule(const std::string& rule, double probability)
{
	rules.emplace_back(rule, probability);
}
bool ReplacementRule::check() const
{
	double sum = 0;
	for(const auto& pair : rules) sum += pair.second;
	return sum == 1;
}
const std::string& ReplacementRule::getReplacement() const
{
	if(rules.empty()) return rules[0].first;
	double random = drand48();
	for (const auto& rule : rules)
	{
		random -= rule.second;
		if(random <= 0) return rule.first;
	}
	std::cerr << "total probability below 1\n";
	return rules[0].first;
}
...
namespace
{
	...
	void parse_rules(std::set<char> const& alphabet, std::map<char, ReplacementRule>& rules, stream_parser& parser, bool parse2D)
	{
		parser.skip_comments_and_whitespace();
		...
		while (true)
		{
			...
			while(true)
			{
				...
				if(parser.peekChar() == '=')
				{
					...
					replacement.addRule(rule, probability);
					parser.skip_comments_and_whitespace();
					if(parser.peekChar() == '&')
					{
						parser.getChar();
				...
				{
					replacement.addRule(rule, 1);
					break;
				}
			}
			if(!replacement.check())
				throw LParser::ParserException("Invalid probability distribution in rule specification", parser.getLine(), parser.getCol());
			rules[alphabet_char] = replacement;
...
{
	assert(get_alphabet().find(c) != get_alphabet().end());
	return replacementrules.find(c)->second.getReplacement();
}
